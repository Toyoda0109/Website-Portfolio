...
function CHORD_LOGIC() {
  return (
    <section id="CHORD_LOGIC">
      ...
      <p>
        <span style={{ color: '#ff69b4'}}>[構成音が同じコードの場合1]</span> Augコードは[Faug, C#aug, Aaug]の構成音が全てF, A, C#であるため、「Faugと表示したかったが、Aaugの転回形として表示されてしまった」という状況が発生します。Augコードに関してはSelect Scalesを利用して、キーに関連のあるコードを優先的に表示するようにしていますが、あまり使用する場面は多くありません。
      </p>
      <p>
        <span style={{ color: '#ff69b4'}}>[構成音が同じコードの場合2]</span> C4, E4, G4, B2のような場合、Cmajor/Bと考える方が多いですが、CM7として表示されることもあります。
      </p>
      <p>
        <span style={{ color: '#ff69b4'}}>[メロディと一緒には使用できない]</span> 構成音によって判定しているため、判定プログラムがその音を和音の一部と認識してしまい、正確な和音が出ないことがあります。
      </p>
    </section>
  );
}
